...
downloads <- tbl(con, "python_lib_stats") |> collect()
...
ggplot(total_downloads, aes(x = library, y = total_downloads)) +
  geom_col() +
  geom_text(aes(label = scales::label_number(
    accuracy = 0.1, scale_cut = cut_short_scale()
  ...
  labs(title = "Total downloads for pandas and Polars over the last year")
```

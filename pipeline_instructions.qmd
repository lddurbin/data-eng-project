...
Now let's load the packages we'll need, and the ones that will access the Google API will read the newly-created .Renviron file to retrieve the settings:
...
Now we can create the bucket. Notice that I'm calling a couple of environment variables again below - my GCP project ID and my the zone where I want to host the bucket, both of which are stored in the .Renviron file we created earlier.
...
Let's list all the objects in our bucket:

```{r get_gcs_objects}
objects <- gcs_list_objects(detail = "full")
```
...
Now let's upload our data from GCS into a table within this BQ dataset, assuming the table doesn't already exist:

```{r bq_table}
plotnine_tbl <- bq_table(pypi, "plotnine")
fields <- as_bq_fields(plotnine_stats)
table_exists <- bq_table_exists(plotnine_tbl)

if(table_exists == FALSE) {
  bq_table_create(plotnine_tbl, fields)
}

bq_table <- bq_table(project, dataset, table = NULL, type = "TABLE")
uri <- paste0("gs://", gcs_get_global_bucket(), "/*")
bq_table_load(plotnine_tbl, uri)
```
